...
불가피하게 다른 이들과 협력하는 동안 모듈 사이의 `순환 의존성`(Circular Dependency)을 발견하기 마련이다. 심지어 혼자서 작업할 때도 발생하기도 한다. 일례로 내가 Django 개발을 할 때 서로 다른 views.py 사이에서 순환 의존성을 발견한 기억이 있다.

오늘은 순환 의존성과 그에 대한 해결책을 살펴보도록 한다. 예를 들어 문서를 저장할 위치를 선택하는 대화상자를 보여주는 GUI 어플리케이션을 만든다고 해봊. 대화상자로 표시할 데이터는 이벤트 핸들러의 인수로 설정할 수 있다. 하지만 대화상자는 올바르게 렌더링하는 방법을 알아내려고 사용자 설정과 같은 전역 상태도 읽어야 한다. 즉, 두 개의 모듈이 상대방에 대한 정보를 모두 필요로 하는 것. 전형적인 순환 의존성의 상황이다.
...
문제는 **프로그램에서 사용자의 선호도와 관련된 _prefs_ 객체를 포함하는 app 모듈도 프로그램을 시작할 때 대화상자를 보여주려고 dialog 클래스를 임포트한다는 것이다.**
...
순환 의존성이 발생했다. 메인 프로그램에서 app 모듈을 임포트하려고 하면 예외가 일어난다.
...
**여기서 정확히 어떤 일이 일어나는지, 그리고 마지막 줄에서 왜 정확히 _dialog_ 모듈에서 _show_ 속성이 없다는  _AttributeError_ 가 raise 됐는지 파악해야 한다.** 우리는 dialog.py에서 분명이 show 함수를 정의했기 때문이다.

그를 위해서는 먼저 파이썬의 모듈 임포트 절차를 자세히 알아봐야 한다. 다음은 파이썬에서 깊이 우선 방식으로 모듈을 임포트하면서 실제로 하는 일이다.

중요하니 제대로 살펴보자.

<br>

1. _sys.path_ 에 들어 있는 위치에서 요구한 모듈을 검색한다.
  - sys 내장 모듈의 path는 list로서 파이썬 인터프리터가 임포트할 모듈들을 검색하는 실제 경로가 된다. 모든 임포트할 모듈은 이 리스트 안에 있는 경로에 위치해야 한다.
2. 모듈을 찾았으면 모듈에서 코드를 로드하고 코드가 컴파일되게 한다.
3. 모듈과 관련된 기능이 접근가능하게 할 빈 모듈 객체를 생성한다.
4. 모듈 객체를 sys.modules에 삽입한다.
  - sys.modules는 dict로서 현재 임포트되어 사용가능한 모듈을 이름을 key, 실제 경로를 value해서 저장하고 있다.
5. 빈 객체에 코드를 실행하면서 모듈의 내용(함수, 클래스, 상수)을 정의한다.
...
**_Dialog_ 클래스의 생성자 인자의 기본값을 _None_ 으로 둔 것을 눈여겨보자. 구체적인 저장경로는 나중에 설정 단계(configure)함수에서 진행할 것이기 때문에 저장경로가 빈 상태에서도 대화창이 생성 가능하게 한다.**
...
동적 임포트를 사용해서 dialog 모듈을 재정의하자. 초기화 시전에 dialog 모듈이 app을 임포트하는 대신 dialog.hosw 함수로 런타임에 app 모듈을 임포트한다.

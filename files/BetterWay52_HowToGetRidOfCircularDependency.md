...
문제는 **프로그램에서 사용자의 선호도와 관련된 _prefs(preferences)_ 객체를 포함하는 app 모듈도 프로그램을 시작할 때 대화상자를 보여주려고 dialog 클래스를 임포트한다는 것이다.**
...
**여기서 정확히 어떤 일이 일어나는지, 그리고 마지막 줄에서 왜 정확히 _app_ 모듈에서 _prefs_ 속성이 없다는  _AttributeError_ 가 raise 됐는지 파악해야 한다.** 우리는 app.py에서 분명이 prefs 변수를 정의했기 때문이다.
